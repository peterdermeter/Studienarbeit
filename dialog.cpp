...
void Dialog::keyPressEvent(QKeyEvent *event) {
    ...
    //    if (key == Qt::Key_1 && massCenters.size() > 0 && automaticNavigation) {
    //        imgProc->getAngles(massCenters[0], angle);
    //        imgProc->getMotorData(angle, angleBefore, angleAfter, motor);
    //        sendMotorData(motor.rotation, motor.bending);
    //    }
    //    if (key == Qt::Key_2 && massCenters.size() > 1 && automaticNavigation) {
    //        imgProc->getAngles(massCenters[1], angle);
    //        imgProc->getMotorData(angle, angleBefore, angleAfter, motor);
    //        sendMotorData(motor.rotation, motor.bending);
    //    }
    //    if (key == Qt::Key_3 && massCenters.size() > 2 && automaticNavigation) {
    //        imgProc->getAngles(massCenters[2], angle);
    //        imgProc->getMotorData(angle, angleBefore, angleAfter, motor);
    //        sendMotorData(motor.rotation, motor.bending);
    //    }
    //    if (key == Qt::Key_Tab) {
...
void Dialog::openFrmGrab() {
    if(Epiphan1->Open()/* && Epiphan2->Open()*/) {
        if(image1 = new cv::Mat(grabSize.height(), grabSize.width(), CV_8UC3, Epiphan1->getImageData())) {
        }
